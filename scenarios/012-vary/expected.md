...
前段に共有キャッシュ（`proxy_cache`）を置き、**辞書を持つクライアント → 持たないクライアント**の順に同じ URL を叩いた結果です。

| 経路 | 1 人目（辞書あり）| 2 人目（辞書なし）|
|---|---|---|
| `Vary` なし | 577 バイト・`dcb`・MISS | **577 バイト・`dcb`・HIT** → 元ファイルと**不一致** |
| `Vary` あり | 577 バイト・`dcb`・MISS | 100,613 バイト・エンコーディングなし・MISS → **一致** |
