// bundle-v2.js — 記事 012 の測定用フィクスチャ（v2 = 次のデプロイ）
...
// ==== app/012-compression/routes.mjs（v2 で追加）====
import { Router } from "express";

export const compressionRouter = Router();

/** 圧縮方式ごとの応答サイズを比べるための固定ペイロードを返す */
compressionRouter.get("/012/payload", (req, res) => {
  const size = Number.parseInt(req.query.size ?? "65536", 10);
  const unit = "compression-dictionary-transport-sample-payload-";
  let body = "";
  while (body.length < size) body += unit;
  res.type("text/plain").send(body.slice(0, size));
});

/** サーバ側の実処理時間を応答ヘッダへ出す（TTFB の見かけと実際を分けるため）*/
compressionRouter.get("/012/think", (req, res) => {
  const ms = Number.parseInt(req.query.ms ?? "200", 10);
  const started = process.hrtime.bigint();
  setTimeout(() => {
    const elapsed = Number(process.hrtime.bigint() - started) / 1e6;
    res.set("Server-Timing", `app;dur=${elapsed.toFixed(1)}`);
    res.type("text/html").send("<!doctype html><title>think</title><p>done</p>");
  }, ms);
});
